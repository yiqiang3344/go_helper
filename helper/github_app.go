...
func GetToken(w http.ResponseWriter, r *http.Request, code string, state string, redirectUri string) (string, error) {
	//重新获取token
	if redirectUri == "" {
		redirectUri = "http://go.sidney.yi:8000/tool"
	}
	...
	WriteLog(data["access_token"][0], "sidneyyi.com/helper/github_app.go::GetToken")

	return data["access_token"][0], nil
}
